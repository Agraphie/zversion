...
#printf "\n"
#printf "'CVE-2014-0088 (1.5.10)'; `jq '.Agents[] | select(.Vendor=="nginx" and .CanonicalVersion == "0001000500100000" and .Version != "") | .Version' $1 | wc -l`"
#printf "\n"
...
#printf '%s\n' '-----------------------'
#
nginxVersions=($(jq --arg meh "$meh" '.Agents[] | select(.Vendor=="nginx" and .CanonicalVersion != "") | .CanonicalVersion' $1))

for i in "${nginxVersions[@]}"
do
    echo $i
    if [ "$i" -ge "0001000300150000" ]; then
        echo "meh"
    fi
done
#meh=0
#jq --arg meh "$meh" '.Agents[] | select(.Vendor=="nginx" and .CanonicalVersion != "") | if .CanonicalVersion >= "0001000300150000" and .CanonicalVersion <= "0001000500110000" then "{$meh}"
#elif .CanonicalVersion == "0001000500100000" then "CVE-2014-0088"
#elif .CanonicalVersion >= "0001000300090000" and .CanonicalVersion <= "0001000400000000" then "CVE-2013-2028"
#elif ((.CanonicalVersion >= "0001000100030000" and .CanonicalVersion <= "0001000100180000") or (.CanonicalVersion >= "0001000000070000" and .CanonicalVersion <= "0001000000140000")) then "CVE-2012-2089"
#elif .CanonicalVersion >= "0000000100000000" and .CanonicalVersion <= "0001000100160000" then "CVE-2012-1180"
#elif .CanonicalVersion >= "0000000100000000" and .CanonicalVersion <= "0001000100160000" then "CVE-2009-3555"
#elif .CanonicalVersion >= "0000000100000000" and .CanonicalVersion <= "0000000800140000" then "CVE-2009-2629"
#elif .CanonicalVersion >= "0000000100000000" and .CanonicalVersion <= "0000000800130000" then "CVE-2009-3896"
#else  (meh=\$meh|tonumber+1)  end' $1
#
#echo $meh